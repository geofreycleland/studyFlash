...
function showFeedback(isCorrect, selected){const q=QUESTIONS[current]; feedback.classList.remove('hidden'); const selectedLetters=selected.map(i=>String.fromCharCode(65+i)).join(', ') || 'None'; feedback.innerHTML=`<p class="result ${isCorrect?'correct':'wrong'}">${isCorrect?'Correct':'Not quite'}</p><p><b>Your answer:</b> ${selectedLetters}<br><b>Correct answer:</b> ${q.answerText}</p><div class="explainBlock"><h3>Why the right answer is right</h3><p>${q.whyRight}</p></div><div class="explainBlock"><h3>Why the other answers are wrong</h3><div class="wrongList">${q.whyWrong.map(w=>`<div class="wrongItem"><b>${w.choice}.</b> ${w.reason}</div>`).join('')}</div></div><div class="explainBlock"><h3>Services used in this question</h3><div class="serviceGrid">${q.services.length?q.services.map(s=>`<div class="service"><b>${s.name}</b>${s.definition}</div>`).join(''):'<p>No major AWS service names were detected for this question.</p>'}</div></div>`; submitBtn.classList.add('hidden'); nextBtn.classList.remove('hidden');}
